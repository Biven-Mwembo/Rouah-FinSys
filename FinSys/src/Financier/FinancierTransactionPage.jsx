...
  const token = localStorage.getItem("token");
    ...
    // Obtenir le nom de l'utilisateur (fonction réutilisée)
...
    const userTxCounts = {};
    const userDetails = {}; // NOUVEAU: Stocke les noms complets
    // Calculate transaction count per user
    approvedTx.forEach(tx => {
      const userId = tx.user_id;
      if (userId) {
          userTxCounts[userId] = (userTxCounts[userId] || 0) + 1;
          // Stocke le nom complet pour l'utilisateur
          if (tx.user && tx.user.name && tx.user.surname) {
              userDetails[userId] = `${tx.user.name} ${tx.user.surname}`;
          }
      }
    });
...
        return {
          id: userId,
          // MODIFIÉ: Utilise le nom complet si disponible, sinon l'ID
          name: userDetails[userId] || userId, 
          txCount: count,
        };
      })
...
        tx.id,
        getUserName(tx), // Affiche le nom complet
        formatDate(tx.date),
        tx.amount,
        tx.currency,
        tx.channel,
...
                          <strong>{user.name}</strong> {/* Affiche le nom complet/ID */}
...
                  <td data-label="Utilisateur">{getUserName(tx)}</td> {/* Affiche le nom complet */}
